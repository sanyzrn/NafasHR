...
def list_users(
    ...
    items = list(db.scalars(query.order_by(_user_order_by(sort_by, sort_dir), User.id).limit(limit).offset(offset)))
...
def export_users_excel(
    ...
    query = _apply_user_filters(select(User), role=role, q=q, is_active=is_active)
    users = list(db.scalars(query.order_by(_user_order_by(sort_by, sort_dir), User.id)))
