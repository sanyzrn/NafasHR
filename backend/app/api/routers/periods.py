...
def delete_period(
    ...
    # Lock the period so concurrent inserts referencing it cannot race deletion.
    period = db.scalar(
        select(EvaluationPeriod).where(EvaluationPeriod.id == period_id).with_for_update()
    )
    if period is None:
        raise HTTPException(status_code=404, detail="دوره ارزیابی یافت نشد")
    count = db.scalar(
    ...
    if count:
        raise HTTPException(
            status_code=409,
            detail=f"این دوره {count} پرونده ارزیابی دارد و قابل حذف نیست؛ می‌توانید دوره را ببندید.",
        )
    snapshot = {
        "id": period.id, "name": period.name, "status": period.status.value,
        "starts_on": str(period.starts_on), "ends_on": str(period.ends_on),
    }
    db.delete(period)
    try:
        db.flush()
        ...
        db.rollback()
        raise HTTPException(status_code=409, detail="این دوره دارای سوابق وابسته است و قابل حذف نیست") from exc
    log_event(db, actor_user_id=current_user.id, event_type="period_deleted", old_value=snapshot)
    db.commit()
