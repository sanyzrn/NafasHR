...
def require_own_personnel(current_user: CurrentUser = Depends(get_current_user)) -> CurrentUser:
    """Personal records are available to linked personnel, except system admins.

    HR and supervisors retain personal access independently of their work role.
    Each endpoint still scopes records to the authenticated personnel ID.
    Submission eligibility is checked separately by may_self_assess.
    """
    if current_user.role == UserRole.support:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="بخش خودارزیابی و کارنامهٔ شخصی برای ادمین کل فعال نیست.",
        )
