...
function PeopleManagementRedirect() {
  ...
  if (loading) return <PageFallback />;
  const canManageAccounts = user?.role === "hr" || can("manage_users");
  return (
    <Navigate
      to={canManageAccounts ? "/hr/people/accounts" : "/hr/people/personnel"}
      replace
    />
  );
}
      ...
      <Routes>
        ...
        <Route element={<ProtectedRoute />}>
          <Route element={<Layout />}>
            ...
            <Route element={<ProtectedRoute allowedRoles={["employee", "unit_supervisor", "hr"]} />}>
