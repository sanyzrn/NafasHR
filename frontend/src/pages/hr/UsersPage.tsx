...
export function UsersPage({ showPersonnelTab = true }: { showPersonnelTab?: boolean } = {}) {
  ...
  const listParams = {
    sort_by: sorting.sort ? ["username", "display_name", "role"][sorting.sort.column] : undefined,
  ...
  async function toggleActive(u: AppUser) {
    if (u.is_active) {
      const ok = await confirm({
        title: `غیرفعال کردن «${u.display_name || u.full_name || u.username}»؟`,
...
export function EditUserModal({
  ...
  return (
    <Modal
      title={`ویرایش کاربر: ${user.display_name || user.full_name || user.username}`}
      onClose={onClose}
      footer={
        <>
