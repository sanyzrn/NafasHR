...
export function Table({
  ...
  const sortButton = (header: ReactNode, column: number) => (
    <button type="button" onClick={() => onSort?.(column)}
      ...
      {header}
      <span aria-hidden="true" className={sort?.column === column ? "text-pulse-700" : "text-gray-400"}>
      ...
      <table className="w-full text-sm">
        <thead>
          <tr className="border-b border-gray-200">
            {headers.map((h, i) => (
              <th key={i} aria-sort={onSort && sortableColumns.includes(i) ? (sort?.column === i ? (sort.direction === "asc" ? "ascending" : "descending") : "none") : undefined} className="px-3 py-2 text-right text-xs font-semibold text-gray-500">
                {onSort && sortableColumns.includes(i) ? sortButton(h, i) : h}
              </th>
            ))}
          </tr>
        </thead>
  ...
  const content = (
    <>
      ...
      {asCards && onSort && sortableColumns.length > 0 && (
        <div className="mb-3 flex flex-wrap items-center gap-2 text-xs text-gray-500" aria-label="مرتب‌سازی">
          <span>مرتب‌سازی:</span>
          {sortableColumns.map((column) => <span key={column}>{sortButton(headers[column], column)}</span>)}
        </div>
      )}
      {rows.length === 0 ? (
