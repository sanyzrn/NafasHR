...
REM  fixed JWT_SECRET_KEY, SEED_DEMO_DATA=false and
...
REM Never seed demo users/personnel when launching an existing database.
...
REM A copied venv may point to Python on a different computer. Prefer the
REM working local environment when present; never delete either environment.
if exist "%BACKEND%\.venv-local\Scripts\python.exe" (
    "%BACKEND%\.venv-local\Scripts\python.exe" -c "import sys" >nul 2>nul
    if not errorlevel 1 set "VENV=%BACKEND%\.venv-local"
)
...
if not exist "%ENV_FILE%" (
    ...
    >>"%ENV_FILE%" echo SEED_DEMO_DATA=false
    >>"%ENV_FILE%" echo.
...
if "!BACKEND_ALREADY_RUNNING!"=="0" start "DbsPulse Backend (port 8000)" /D "%BACKEND%" cmd /k ""%PY%" -m uvicorn app.main:app --reload --host 0.0.0.0 --port 8000"
...
echo  Existing backend settings and database records are preserved.
echo.
